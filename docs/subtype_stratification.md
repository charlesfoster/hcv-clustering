...
The clustering threshold (a provisional 0.03 for the default full `e1-e2` region; see [`docs/threshold_rationale.md`](threshold_rationale.md)) is intended for within-subtype comparisons. In a mixed matrix it is either irrelevant (no cross-subtype pairs will link, so the threshold is redundant) or — if there are alignment artefacts producing spuriously low cross-subtype distances — it becomes a source of false positive clusters.
